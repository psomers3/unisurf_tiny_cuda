...
import model
from model.common import get_tensor_values, sample_patch_points, arange_pixels
...
class Trainer(object):
    ...
    def __init__(self, _model: model.Renderer, optimizer, cfg, device=None, **kwargs):
        self.model = _model
    ...
    def render_visdata(self, data, resolution, it, out_render_path):
        (img, mask, world_mat, camera_mat, scale_mat, img_idx) = self.process_data_dict(data)
        h, w = resolution
        ...
        with torch.no_grad():
            ...
            Image.fromarray(img_out.astype(np.uint8)).convert("RGB").\
                save(os.path.join(out_render_path, '%04d_unisurf.png' % img_idx))
        ...
        with torch.no_grad():
            ...
            Image.fromarray(img_out.astype(np.uint8)).convert("RGB").\
                save(os.path.join(out_render_path, '%04d_phong.png' % img_idx))
    ...
    def compute_loss(self, data, eval_mode=False, it=None):
        ...
        if n_points >= h*w:
            ...
            p, pix = sample_patch_points(batch_size,
                                         n_points,
                                         patch_size=1.,
